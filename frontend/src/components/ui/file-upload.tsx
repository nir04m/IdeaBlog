...
type FileUploadProps = {
  /** Called with hosted URL (e.g., R2) after upload finishes */
  onUploadComplete?: (url: string) => void;
  /** Optional: a function that uploads the file and returns a public URL */
  uploader?: (file: File) => Promise<string>;
};

export const FileUpload: React.FC<FileUploadProps> = ({ onUploadComplete, uploader }) => {
  ...
  const handlePickedFile = async (picked: File | null) => {
    ...
    try {
      if (uploader) {
        const hostedUrl = await uploader(picked);
        onUploadComplete?.(hostedUrl);
      } else {
        // Fallback: local preview URL (not for prod storage)
